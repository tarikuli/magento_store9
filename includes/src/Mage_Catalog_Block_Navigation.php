...
class Mage_Catalog_Block_Navigation extends Mage_Core_Block_Template {
	...
	public function getSideImages() {
		...
		try {
			...
			$sql = $connection->query("select * from $tblname where status='0' and groupname='side_banner' ORDER BY imageorder ASC ");
			...
			while ($row1 = $sql->fetch()) {
				...
				$images[] = array('image' => $imgurl, 'url' => $url, "title" => $title, "description" => $description);
			}
		} catch (Exception $e) {
		}
		return $images;
