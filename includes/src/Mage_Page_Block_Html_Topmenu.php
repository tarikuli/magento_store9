...
class Mage_Page_Block_Html_Topmenu extends Mage_Core_Block_Template {
	...
	function getMenuSubCategories($parentId) {
		$data = array();
	...
	function getAllCategories() {
		...
		if (!empty($ids)) {
			...
			foreach ($ids as $id) {
				...
				/*if ($category['level'] == 4) {
					$data[$category['parentId'] . "-level2"]['items'][$category['parentId'] . "-level3"]['items'] = $category;
				}*/

				/*if ($category['level'] == 5) {
					array_push($data['model'][$category['parentId']]['items'], $category);
				}*/
	...
	public function getMenuHtml($attr = "", $is_option = "true", $selector = "", $sideimage = null, $title = null) {
		...
		$sideimage = $this->getMenuImage($attr);
		if ($columns < 4 && $sideimage) {
			$html .= '<div class="matchHeight col-menu-items col-image col-md-' . $imgcol . '"><a href="' . $sideimage['url'] . '"><img src="' . $sideimage['image'] . '" class="img-responsive" alt="usmap"></a></div>';
		}
	...
	public function getCategoryMenuHtml($category, $items) {
		...
		foreach ($items as $key => $item) {
			...
			$html .= '<li class="search-link"> <a title="" href="' . $item['url'] . '">' . $item['name'] . '</a>';

			//check for subcategory
			$children = $this->getMenuSubCategories($category['id']);
			if ($children && count($children) > 0) {
				$html .= $this->getCategoryMenuHtml($category, $children);
			}
		...
		if ($columns < 4 && $sideimage) {
			$html .= '<div class="matchHeight col-menu-items col-image col-md-' . $imgcol . '"><a title="' . $category['title'] . '" href="' . $category['url'] . '"><img src="' . $sideimage . '" class="img-responsive" alt="usmap"></a></div>';
		}
