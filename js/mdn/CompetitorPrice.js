...
CompetitorPrice.prototype = {
    ...
    addOffersInProductView: function(productId, barcode, channel)
    {
        competitorPriceObj.lastProductId = productId;
        var params = "action=prices&products=" + '{"' + productId + '":{"channel":"' + channel +'","ean":"' + barcode +'"}}';
        ...
        new Ajax.Request(
            this.hostUrl,
            {
                ...
                onSuccess: function onSuccess(transport)
                {
                    ...
                    if (response.offers[competitorPriceObj.lastProductId].status == 'ASSOCIATED')
                    {
                        var offers = response.offers[competitorPriceObj.lastProductId].offers;
                        var currency = response.offers[competitorPriceObj.lastProductId].currency;
                        ...
                        var channelName = response.offers[competitorPriceObj.lastProductId].channel;
                        ...
                        if (response.offers[competitorPriceObj.lastProductId].link)
                            content += '<a target="_blank" href="' + response.offers[competitorPriceObj.lastProductId].link + '">'
                        content += channelName;
                        if (response.offers[competitorPriceObj.lastProductId].link)
                            content += '</a>';
