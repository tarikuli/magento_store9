...
class Magestore_Webpos_Helper_Product extends Mage_Core_Helper_Abstract {
    ...
    public function getProductHtml(Mage_Catalog_Model_Product $product, $extraClass = '') {
        ...
        $specialPriceDate = Mage::app()->getLocale()->isStoreDateInInterval($store, $product->getSpecialFromDate(), $product->getSpecialToDate());
        if (isset($specialPrice)&& $specialPriceDate) {
            $baseCurrencyCode = Mage::app()->getStore()->getBaseCurrencyCode();
            $currentCurrencyCode = Mage::app()->getStore()->getCurrentCurrencyCode();
            $specialPrice = Mage::helper('directory')->currencyConvert($specialPrice, $baseCurrencyCode, $currentCurrencyCode);
            $finalPrice = floatval($specialPrice);

            $originalPrice = Mage::helper('directory')->currencyConvert($originalPrice, $baseCurrencyCode, $currentCurrencyCode);
        ...
        if (isset($specialPrice) && $specialPriceDate) {
            $html .= "<div class='prd_price'><span class='prd_price_reg'>" . $store->formatPrice($originalPrice) . "</span><span class='prd_price_special'>" . $priceDisplay . "</span></div>";
        }else{
            $html .= "<div class='prd_price'><span class='prd_price_special'>" . $priceDisplay . "</span></div>";
        }
        $html .= "<img class='product_image' src='' imgpath='" . $imgPath . "'/>
						</div>
    ...
    public function getBundleChilds(Mage_Catalog_Model_Product $product) {
        ...
        foreach ($options->getItems() as $option) {
            ...
            foreach ($selections as $selection) {
                $isInStock = $selection->getStockItem()->getData('is_in_stock');
                $backorders = $selection->getStockItem()->getData('backorders');
                if(!$isInStock && !$backorders){
                    continue;
                }
        ...
        switch ($type) {
            case "select":
                if (count($items) > 0) {
                    ...
                    foreach ($bundleOptionData['items'] as $itemId => $itemData) {
    ...
    public function getCustomOptionValues($option, $optionId = null) {
        $optionsValues = array();
        if ($optionId != null)
    ...
    public function isEnableSearchOnLocal() {
        $config = Mage::getStoreConfig('webpos/offline/search_offline', $this->getStoreId());
