...
class Magestore_Webpos_Adminhtml_PosuserController extends Mage_Adminhtml_Controller_Action {
    ...
    public function saveAction() {
        if ($data = $this->getRequest()->getPost()) {
            ...
            try {
                ...
                $model->save();
                /* Edit by Dante - Trinh Huy Hoang - 07/09/2015 */
                if (Mage::helper('webpos')->isInventoryWebPOS11Active()) {
                    $userId = $this->getRequest()->getParam('id');
                    $warehouseWebposUser = Mage::getModel('inventorywebpos/webposuser')->load($userId, 'user_id');
                    if ($warehouseWebposUser->getId()) {
                        $warehouseWebposUser->setWarehouseId($data['warehouse_id']);
                    } else {
                        $warehouseWebposUser = Mage::getModel('inventorywebpos/webposuser');
                        $warehouseWebposUser->setUserId($model->getId());
                        $warehouseWebposUser->setWarehouseId($data['warehouse_id']);
                        $warehouseWebposUser->setCanCreateShipment(1);
                    }
                    $warehouseWebposUser->save();
                }
                /* End edit by Dante */
    ...
    public function deleteAction() {
        if ($this->getRequest()->getParam('id') > 0) {
            try {
                if (Mage::helper('webpos')->isInventoryWebPOS11Active()) {
                    $warehouseWebposUsers = Mage::getModel('inventorywebpos/webposuser')->getCollection()
                            ->addFieldToFilter('user_id', array('eq' => $this->getRequest()->getParam('id')));
                    foreach ($warehouseWebposUsers as $warehouseWebposUser) {
                        $warehouseWebposUser->delete();
                    }
                }
                $model = Mage::getModel('webpos/user');
