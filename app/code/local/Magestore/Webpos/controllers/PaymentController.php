...
class Magestore_Webpos_PaymentController extends Magestore_Webpos_Controller_Action {
    ...
    public function saveDataAction() {
        ...
        $shipping_method = $this->getRequest()->getPost('shipping_method', 'webpos_shipping_webpos_shipping');
        ...
        $oldShippingMethod = '';
        try {
            $oldShippingMethod = $this->getOnepage()->getQuote()->getShippingAddress()->getShippingMethod();
        ...
        try {
            ...
            if($payment_method == 'multipaymentforpos' && $shipping_method != $oldShippingMethod){
                $this->getOnepage()->getQuote()->getPayment()->setData('cashforpos_ref_no', '');
                $this->getOnepage()->getQuote()->getPayment()->setData('ccforpos_ref_no', '');
                $this->getOnepage()->getQuote()->getPayment()->setData('cp1forpos_ref_no', '');
                $this->getOnepage()->getQuote()->getPayment()->setData('cp2forpos_ref_no', '');
                $this->getOnepage()->getQuote()->getPayment()->setData('codforpos_ref_no', '');
            }
        } catch (Exception $e) {
...
//        $upgrandtotal = ($grandTotal % 50 == 0) ? $grandTotal : floor($grandTotal - $grandTotal % 50 + 50);
    ...
    public function reloadOrderAction() {
        ...
        if ($order) {
            ...
            try {
                $reloading = $checkoutSession->getData('reloading_onorder_id');
            ...
            try {
                $order->unhold()->save();
                $order->cancel()->save();
                $checkoutSession->setData('reloading_onorder_id', $order_id);
        ...
        try {
            ...
            if ($order->getId()) {
                ...
                foreach ($payment_changed_values as $payment_code => $value) {
                    ...
                    $values = $helper->getValueForOrder($order, $value);
                    $oldValue = floatval($order->getData($dbFieldName));
                    $oldBaseValue = floatval($order->getData($dbFieldName));
                    $order->setData($dbFieldName, $oldValue + $values['amount']);
                    $order->setData($dbBaseFieldName, $oldBaseValue + $values['base']);

                    if ($payment_code == 'cashforpos') {
                        Mage::helper('webpos/payment')->updateCashTransactionFromOrder($order, $oldValue + $values['amount']);
                    }

                    if($payment_code != 'change'){
                        $paymentFormDbFieldName = $payment_code . '_ref_no';
                        $oldPaymentValue = floatval($payment->getData($paymentFormDbFieldName));
                        $payment->setData($paymentFormDbFieldName, $oldPaymentValue + $values['amount']);
                    }
                }
