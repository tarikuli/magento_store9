    ...
    public function savecartAction() {
        ...
        try {
            ...
            if (isset($productsInfoStr)) {
                ...
                if (is_array($productsInfo) && count($productsInfo) > 0) {
                    foreach ($productsInfo as $productInfo) {
                        ...
                        if (count($data) > 1) {
                            ...
                            if (!empty($storecreditAmount) && is_array($storecreditAmount)) {
                            ...
                            if (isset($data[2])) {
                                ...
                                if ($hasco == true) {
                                    if (!empty($webposNewCustomOption) && is_array($webposNewCustomOption)) {
                                        foreach ($webposNewCustomOption as $prdid => $customOptions) {

                                            foreach ($customOptions as $key => $customOption) {
                                                if (is_array($customOption) && (int) $prdid == $productId_itemId && $key == $index) {
                            ...
                            if (!empty($customOptions) && is_array($customOptions))
                                foreach ($customOptions as $prdid => $customOption) {
                                    if (is_array($customOption) && (int) $prdid == $productId_itemId) {
                                        foreach ($customOption as $optionId => $optionValue) {
                                            $filePath = Mage::getBaseDir('media') . DS . 'webpos' . DS . 'custom_options' . DS . $optionValue;
                                            $subPath = DS . 'media' . DS . 'webpos' . DS . 'custom_options' . DS . $optionValue;
                                            if (is_file($filePath)) {
                            ...
                            if (!empty($bundle_option) && is_array($bundle_option))
                            ...
                            if (!empty($bundle_option_qty) && is_array($bundle_option_qty))
                        ...
                        if ($data['params']['qty'] > 0) {
                            if (Mage::helper('webpos')->isInventoryWebPOS11Active()) { //Magnus
                                ...
                                if ($data['params']['qty'] > $wpQty) {
                                    if (!$backorders) {
                                        $result['errorMessage'] .= $this->__('Can not add items to cart. Because you added %d item(s) %s to cart but it only has %d item(s) in this warehouse.', $data['params']['qty'], $prd_name, $wpQty);
                                        $this->getResponse()->setBody(Zend_Json::encode($result));
                                        return;
                                    } else {
                ...
                if (count($updateItemsData) > 0) {
                    foreach ($updateItemsData as $itemId => $params) {
                        if (Mage::helper('webpos')->isInventoryWebPOS11Active()) { /* Magnus */
                            ...
                            if ($newQty > $wpQty) {
                                if (!$backorders) {
                                    $result['errorMessage'] .= $this->__('Can not add items to cart. Because you added %d item(s) %s to cart but it only has %d item(s) in this warehouse.', $newQty, $prd_name, $wpQty);
                                    $this->getResponse()->setBody(Zend_Json::encode($result));
                                    return;
                                } else {
        ...
        if (isset($shippingAddress)) {
            ...
            if ($shippingMethod == '' && $helperPayment->isWebposShippingEnabled()) {
                $shippingAddress->setShippingMethod('webpos_shipping_webpos_shipping');
                $quote->setTotalsCollectedFlag(false)->collectTotals();
                $this->getOnepage()->saveShippingMethod('webpos_shipping_webpos_shipping');
            }
        }
            ...
            if ($item->getId() == $itemId) {
                ...
                if (Mage::helper('webpos')->isInventoryWebPOS11Active()) {
                    ...
                    if(strpos($item->getProduct()->getSku(),'webpos-customsale') === false) {
                        ...
                        if ($qty > $wpQty) {
                            if (!$backorders) {
                                $message = $this->__('Can not add items to cart. Because you added %d item(s) %s to cart but it only has %d item(s) in this warehouse.', $qty, $prd_name, $wpQty);
                            } else {
                                $item->setQty($wpQty);
                                $message =  $this->__('You added %d item(s) %s to cart but it only has %d item(s) in this warehouse.', $qty, $prd_name, $wpQty);
                            }
                        }
                    } else {
                        $item->setQty($qty);
                    }
                    /* Endl edited by Magnus */
                }else{
                    $item->setQty($qty);
                }
