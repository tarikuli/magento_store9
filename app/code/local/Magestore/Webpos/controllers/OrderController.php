    ...
    public function saveOrderAction() {
        ...
        if (!$isVirtual) {
            ...
            if ((!isset($shipping_method) || $shipping_method == '') && $helperPayment->isWebposShippingEnabled())
                $shipping_method = 'webpos_shipping_webpos_shipping';
            try {
                /*
        ...
        if (!$error) {
            ...
            if ($redirectUrl) {
                ...
                $reloading_order_id = Mage::getModel('checkout/session')->getData('reloading_onorder_id');
                ...
                if ($totalDue <= 0) {
                    ...
                    $currentCurrencyCode = $order->getData('order_currency_code');
                    $baseCurrencyCode = $order->getData('base_currency_code');
                    $totalPaid = 0;
                    if (isset($payment['ccforpos_ref_no'])) {
                        $baseAmount = Mage::helper('directory')->currencyConvert($payment['ccforpos_ref_no'], $currentCurrencyCode, $baseCurrencyCode);
                        $order->setData('webpos_ccforpos', $payment['ccforpos_ref_no']);
                        $order->setData('webpos_base_ccforpos', $baseAmount);
                        $totalPaid += $payment['ccforpos_ref_no'];
                    }
                    if (isset($payment['cp1forpos_ref_no'])) {
                        $baseAmount = Mage::helper('directory')->currencyConvert($payment['cp1forpos_ref_no'], $currentCurrencyCode, $baseCurrencyCode);
                        $order->setData('webpos_cp1forpos', $payment['cp1forpos_ref_no']);
                        $order->setData('webpos_base_cp1forpos', $baseAmount);
                        $totalPaid += $payment['cp1forpos_ref_no'];
                    }
                    if (isset($payment['cp2forpos_ref_no'])) {
                        $baseAmount = Mage::helper('directory')->currencyConvert($payment['cp2forpos_ref_no'], $currentCurrencyCode, $baseCurrencyCode);
                        $order->setData('webpos_cp2forpos', $payment['cp2forpos_ref_no']);
                        $order->setData('webpos_base_cp2forpos', $baseAmount);
                        $totalPaid += $payment['cp2forpos_ref_no'];
                    }
                    if (isset($payment['codforpos_ref_no'])) {
                        $baseAmount = Mage::helper('directory')->currencyConvert($payment['codforpos_ref_no'], $currentCurrencyCode, $baseCurrencyCode);
                        $order->setData('webpos_codforpos', $payment['codforpos_ref_no']);
                        $order->setData('webpos_base_codforpos', $baseAmount);
                        $totalPaid += $payment['codforpos_ref_no'];
                    }
                    if (isset($payment['cashforpos_ref_no'])) {
                        $baseAmount = Mage::helper('directory')->currencyConvert($payment['cashforpos_ref_no'], $currentCurrencyCode, $baseCurrencyCode);
                        $order->setData('webpos_cash', $payment['cashforpos_ref_no']);
                        $order->setData('webpos_base_cash', $baseAmount);
                        $totalPaid += $payment['cashforpos_ref_no'];
                    }
                    $changeAmount = floatval($grandTotal - $totalPaid);
                    if($changeAmount < 0){
                        $baseAmount = Mage::helper('directory')->currencyConvert($changeAmount, $currentCurrencyCode, $baseCurrencyCode);
                        $order->setData('webpos_change', $changeAmount);
                        $order->setData('webpos_base_change', $baseAmount);
                    }
                }
...
//                $ord = $order->setEmailSent(false)->setCustomerEmail($orderCustomerEmail)->queueOrderUpdateEmail(true, '', true);
    ...
    public function savePendingOrder($orderData) {
        ...
        $quote = $cart->getQuote();
        $result = array();
        $shipping_method = (isset($orderData['shipping_method'])) ? $orderData['shipping_method'] : "webpos_shipping_webpos_shipping";
        ...
        if (isset($orderResult) && !empty($orderResult)) {
            ...
            if ($totalDue <= 0) {
                ...
                if (isset($paymentData['cp2forpos_ref_no'])) {
                    $order->setData('webpos_cp2forpos', $paymentData['cp2forpos_ref_no']);
                    $order->setData('webpos_base_cp2forpos', $paymentData['cp2forpos_ref_no']);
                }
