...
class MDN_ExtensionConflict_Adminhtml_ExtensionConflict_AdminController extends Mage_Adminhtml_Controller_Action
{
	...
	public function LogsAjaxAction(){
		$this->getResponse()->setBody($this->getTemplate('Logs','Logs'));
	}

	private function getTemplate($template,$blockName = 'List'){

		$extName = 'ExtensionConflict';

		return $this->getLayout()
				->createBlock($extName.'/'.$blockName)
				->setTemplate($extName.'/'.$template.'.phtml')
				->toHtml();
	}

	public function DlFileAjaxAction()
	{
		$content = '';
		...
		if($path) {
			$path = mage::helper('ExtensionConflict/File')->decodeFilePathForAjax($path);
	...
	public function LoadFileAjaxAction()
	{
		$file = '';
		...
		if($path) {
			$path = mage::helper('ExtensionConflict/File')->decodeFilePathForAjax($path);
			$file = mage::helper('ExtensionConflict/File')->getFile($path);
		}
