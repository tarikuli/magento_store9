...
class Excellence_Ajax_IndexController extends Mage_Checkout_CartController {
	...
	public function productsearchAction() {
		...
		try {
			$response['items'] = $this->getMenuSubCategories($params['category-id']);
	...
	public function productsearchattrAction() {
		$response = array();
		$params = $this->getRequest()->getParams();
	...
	public function createsubcategoriesAction() {
		...
		$response['status'] = 'SUCCESS';
		$response['message'] = $count . ' Sub categories created - ' . $parentCategory->getPath() . ' - ' . $parentId;
