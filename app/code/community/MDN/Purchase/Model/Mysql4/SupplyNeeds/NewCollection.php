...
class MDN_Purchase_Model_Mysql4_SupplyNeeds_NewCollection extends Mage_Catalog_Model_Resource_Product_Collection {
    ...
    public function addAttributeToFilter($attribute, $condition = null, $joinType = 'inner')
    {
        ...
        switch($attribute) {
            ...
            case 'manufacturer_id':
				$manufacturerCode = mage::getModel('AdvancedStock/Constant')->GetProductManufacturerAttributeCode();
				$generatedName = 'at_'.$manufacturerCode;
                $replacementArray = array($attribute => '`'.$generatedName.'`.`value`');
                $conditionSql = $this->_getConditionSql($this->getExpressionWithAttributes(
    ...
    {
        $originalQuery = clone $this->getSelect();
        $countSelect = ('SELECT count(*) from ( '.$originalQuery.') AS original_table');
        $countSelect = str_replace("`e`.*,", "e.entity_id,", $countSelect);
        $countSelect = str_replace("`cisi`.*,", "", $countSelect);
        $countSelect = str_replace("`cis`.*,", "", $countSelect);
        $countSelect = str_replace("`pa`.*,", "", $countSelect);
        $countSelect = str_replace("`sh`.*,", "", $countSelect);
        return $countSelect;
    }
