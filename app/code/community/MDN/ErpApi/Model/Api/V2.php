...
class MDN_ErpApi_Model_Api_V2 extends Mage_Api_Model_Resource_Abstract {
    ...
    */
            ...
            if($po){
                ...
                foreach($collection as $pop){
                    ...
                    $results[] = array(
                        ...
                        'pop_product_ean' => $this->formatStringForSoap($barcode),
            ...
            if($order && $order->getId() > 0){
                ...
                foreach ($order->getItemsCollection() as $item) {
                    ...
                    if($productId > 0){
                        ...
                        $p['product_shelf_location'] = (string)$stockItem->getshelf_location();
     ...
     * @return boolean
     */
    public function commitPacking($orderId,$warehouseId,$operatorId = 1, $weight = null){
        ...
        if($orderId != null && $orderId >0 && $orderId != null && $orderId >0 && $operatorId != null && $operatorId >0){
            ...
            if($toPack){
                ...
                if($weight && $weight>0){
                    $orderToPrepareItem = mage::getModel('Orderpreparation/ordertoprepare')
                        ->getCollection()
                        ->addFieldToFilter('order_id', $orderId)
                        ->addFieldToFilter('user', Mage::helper('Orderpreparation')->getOperator())
                        ->addFieldToFilter('preparation_warehouse', mage::helper('Orderpreparation')->getPreparationWarehouse())
                        ->getFirstItem();
                    if($orderToPrepareItem && $orderToPrepareItem->getId()){
                        $orderToPrepareItem->setreal_weight($weight)->save();
                    }
                }
                $order = Mage::getModel('sales/order')->load($orderId);
    ...
    private function printDocumentbyType($type,$mode,$warehouseId,$userId){
        ...
        try {
            ...
            if($warehouseId >0 && $userId >0 ){
                ...
                //print the requested document depending of the printing Mode
