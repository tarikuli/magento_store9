...
class MDN_SalesOrderPlanning_Block_Widget_Grid_Column_Renderer_OrderPlanning extends Mage_Adminhtml_Block_Widget_Grid_Column_Renderer_Abstract 
{
	...
	public function getPlanning($row){
		...
		$helper = mage::helper('SalesOrderPlanning/Planning');
		$planning = $helper->getPlanningForOrderId($orderId);
		
		//update the planning is necessary
		if ($planning != null && $planning->getId() > 0 
		    && $planning->getpsop_need_update() == 1)
		{
			$helper->updatePlanning($orderId);
			//A relaod of the planning is necessary after the update
			$planning = $helper->getPlanningForOrderId($orderId);
		}
