...
{
    /**
     * check version magento 1.9.x
     */
    protected function _checkVersion19x() {
        $version = Mage::getVersionInfo();
        return ($version['major'] == '1' and $version['minor'] == '9')?true:false;
    }

    /**
     * sendNewOrderEmail new version(1.9.x)
     * @return Mage_Sales_Model_Order|void
     */
    ...
    public function queueNewOrderEmail($forceMode = false)
    {
    	if(!Mage::getStoreConfig('pdfpro/config/enabled')) {
            return ($this->_checkVersion19x())?parent::queueNewOrderEmail() : parent::sendNewOrderEmail();
        }
    	switch(Mage::getStoreConfig('pdfpro/config/order_email_attach')){
    		case VES_PdfPro_Model_Source_Attach::ATTACH_TYPE_NO:
    			if($this->_checkVersion19x()){
                    return parent::queueNewOrderEmail();
                } else {
                    return parent::sendNewOrderEmail();
                }
    		case VES_PdfPro_Model_Source_Attach::ATTACH_TYPE_ADMIN:
    			$this->sendNewOrderEmailForAdmin(true,$forceMode);
    			$this->sendNewOrderEmailForCustomer(false, $forceMode);
    			return $this;
    		case VES_PdfPro_Model_Source_Attach::ATTACH_TYPE_CUSTOMER:
    			$this->sendNewOrderEmailForAdmin(false,$forceMode);
    			$this->sendNewOrderEmailForCustomer(true,$forceMode);
    			return $this;
    	}
    ...
    public function sendNewOrderEmailForCustomer($attachPdfFile = true, $forceMode = false){
    	$storeId = $this->getStore()->getId();
    ...
    public function sendNewOrderEmailForAdmin($attachPdfFile = true,$forceMode = false){
    	$storeId = $this->getStore()->getId();
