...
header('Access-Control-Allow-Origin: https://batch.internetpostage.com'); 
...
{
	// Open the XML output and root in case of exception as we dont write it
	writeXmlDeclaration();
...
//Check to see if the order already shipments
	//if($order->hasShipments())
		//SDC Code Change 6/13/2015 to Avoid an error
	$shipmentsColl = $order->getShipmentsCollection();
	if($shipmentsColl != false && $shipmentsColl->count() > 0 )
	{
	...
	else 
	{
		...
		//SDC Code Change 6/13/2015 to Avoid an error while calling prepareshipment.
		$itemscoll = $order->getItemsCollection();
		$itemscollArr = array();
		foreach ($items as $itemscoll)
		{
				array_push($itemscollArr, $items);
		}
		$shipment = $order->prepareShipment($itemscollArrs);
		$shipment->register();
	}   
